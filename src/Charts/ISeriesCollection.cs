...
internal sealed class SeriesCollection : ISeriesCollection
{
    ...
    public int Count => SeriesListCore().Count;

    public ISeries this[int index] => SeriesListCore()[index];

    public void RemoveAt(int index)
    {
        CSerList()[index].Remove();
    }

    public IEnumerator<ISeries> GetEnumerator()
    {
        return SeriesListCore().GetEnumerator();
    }
    ...
    private List<ISeries> SeriesListCore()
    {
        ...
        foreach (var cXChart in cXCharts)
        {
            ...
            seriesList.AddRange(cSerList.Select(cSer => new Series(chartPart, cSer, seriesChartType)));
        }
